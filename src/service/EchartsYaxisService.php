 ...
 * 
 */
class EchartsYaxisService extends Base implements MainModelInterface {
    ...
    protected static $mainModelClass = '\\xjryanse\\echarts\\model\\EchartsYaxis';

    public static function getYaxis($echartId) {
        $con[] = ['echart_id', '=', $echartId];
        $info = self::staticConFind($con);
        $info['data'] = json_decode($info['data'], JSON_UNESCAPED_UNICODE);

        return Arrays::getByKeys($info, ['type', 'data']);
    }
